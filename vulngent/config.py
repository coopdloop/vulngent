...
class Settings(BaseSettings):
    ...
    # --- Agent economics (usage cost + value modelling) ---
    # USD per 1M tokens for the configured model; used to price recorded token usage.
    agent_cost_input_per_mtok: float = 3.0
    agent_cost_output_per_mtok: float = 15.0
    # Fully-loaded analyst cost used to value the work the agents did instead.
    agent_analyst_hourly_rate: float = 95.0
    # Analyst minutes assumed per ledger-changing agent action (outreach, linking, status).
    agent_minutes_per_action: float = 12.0
